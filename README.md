...
- [配置文件](#配置文件)
- [日志解析规则](#日志解析规则)
- [Ecliptica 系世界适配](#ecliptica-系世界适配)
- [工作原理](#工作原理)
- [目录结构](#目录结构)
- [测试与验证](#测试与验证)
- [兼容性与限制](#兼容性与限制)
...
> 官方世界会输出；实测 `男生女生向前冲` 会输出阶段、Boss、伤害、印记等全部日志，
> 但**完全不输出 `ownership`**，因此在该世界里「目标」显示 `—`、切换次数恒为 0。
> 这是日志里没有这份数据，不是解析或去重的问题——
> 详见 [Ecliptica 系世界适配](#ecliptica-系世界适配)。
...
官方世界名是 `Ecliptica …`，但社区存在**非官方改版**（例如 `男生女生向前冲`）：
它们复用同一套 Udon 脚本，战斗日志的 `ECLIPTICA …` 前缀完全一致，只有房间名不同。

程序**不靠房间名硬编码**，两层保证：

1. **世界别名表** —— 内置 `ecliptica` 与 `男生女生向前冲`，可在 `config.ini` 的
   `world_names` 继续追加；命中即立刻开局。
2. **惰性开局** —— 只要出现任意 `ECLIPTICA …` 系战斗日志就自动补开一局。
   因此即使是从未见过的换皮世界名，甚至 HUD 从日志尾部才开始跟随、完全没看到房间行，
   也能正常工作。

### 不同改版的日志差异

阶段、Boss、伤害、承伤、死亡、印记这些事件在所有已知 Ecliptica 系世界里
**格式完全一致**，因此统计部分通用。但有一处已知差异：

| 日志行 | 官方 `Ecliptica` | `男生女生向前冲` |
|---|---|---|
| `ECLIPTICA - now in stage:` | ✅ | ✅ |
| `ECLIPTICA - now fighting boss:` | ✅ | ✅ |
| `Dealing` / `damage has been taken` | ✅ | ✅ |
| `Boss X dead` + `STRIKE DMG` | ✅ | ✅ |
| `spawn token` / `saving SESSION ID` | ✅ | ✅ |
| **`ownership of X transferred to Y`** | ✅ | ❌ **完全没有** |
| `ECLIPTICA MASTER Setting SESSION ID to N` | ❌ | ✅（不影响统计）|

也就是说，**目标（仇恨）追踪依赖世界是否输出 `ownership`**。在 `男生女生向前冲`
里，「目标」会显示 `—`、切换次数恒为 0，其余统计全部正常。若该世界后续版本加入
`ownership`，程序无需修改即可自动开始显示目标。
...
| 目标追踪 | 官方世界 1544 条 `ownership` → 逐对象去重后采纳 **1342** 条；`男生女生向前冲` 0 条 → 显示 `—` |
| 非官方改版 | **实时日志**核对：阶段、Boss、伤害、印记全部正常解析 |
